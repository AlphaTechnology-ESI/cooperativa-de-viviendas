...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function agregarHoraAlListado(hora) {
        ...
        const botonComprobante = hora.comprobante_nombre
            ? `<button class="btn btn-small btn-secondary" onclick="window.location.href='${API_URL}/cooperativa-de-viviendas-apis/endpoint/dashboard/user/descargar_comprobante.php?id=${hora.id_jornada}'" style="margin-left:10px;">📄</button>`
            : "";
    ...
    async function cargarHoras() {
        try {
            const res = await fetch(`${API_URL}/cooperativa-de-viviendas-apis/endpoint/dashboard/user/listar_horas.php`, {
                method: "POST",
    ...
    btn.addEventListener("click", async function () {
        ...
        try {
            const res = await fetch(`${API_URL}/cooperativa-de-viviendas-apis/endpoint/dashboard/user/horas.php`, {
                method: "POST",
                body: formData
            });
            const result = await res.json();
