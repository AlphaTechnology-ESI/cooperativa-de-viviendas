...
async function abrirModal(id) {
    ...
    ${h.comprobante_nombre ? `
        <div style="margin-top:10px;">
            ...
            <img src="${API_URL}/endpoint/dashboard/admin/descargar_comprobante_admin.php?id=${h.id_jornada}"
                 alt="Comprobante"
                 style="max-width: 300px; display: block; border: 1px solid #ccc; border-radius: 6px; margin: 10px 0;">
            <button class="btn btn-secondary btn-small"
                onclick="window.open('${API_URL}/endpoint/dashboard/admin/descargar_comprobante_admin.php?id=${h.id_jornada}', '_blank')">
...
async function actualizarEstado(estado) {
    ...
    const res = await fetch(`${API_URL}/endpoint/dashboard/admin/cambiar_estado.php`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
